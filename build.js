...
// ── Inject devotions data directly into devotions.html ──────
// Use process.cwd() which is always the repo root on Netlify
var devotionsHtmlPath = path.join(process.cwd(), 'devotions.html');
// Fallback to __dirname if cwd doesn't work
if (!fs.existsSync(devotionsHtmlPath)) {
  devotionsHtmlPath = path.join(__dirname, 'devotions.html');
}
console.log('  📂 Looking for devotions.html at: ' + devotionsHtmlPath);
if (fs.existsSync(devotionsHtmlPath)) {
  var devotHtml = fs.readFileSync(devotionsHtmlPath, 'utf8');
  var dataScript = '<script>window.CMS_DEVOTIONS = ' + JSON.stringify(devotions) + ';</script>';
  // Replace whichever placeholder exists
  if (devotHtml.indexOf('<script>window.CMS_DEVOTIONS') > -1) {
    devotHtml = devotHtml.replace(/<script>window\.CMS_DEVOTIONS[\s\S]*?<\/script>/, dataScript);
  } else if (devotHtml.indexOf('<!-- CMS_DATA_INJECT -->') > -1) {
    devotHtml = devotHtml.replace('<!-- CMS_DATA_INJECT -->', dataScript);
  } else {
    devotHtml = devotHtml.replace('<!-- CMS_DATA_PLACEHOLDER -->', dataScript);
  }
  fs.writeFileSync(devotionsHtmlPath, devotHtml);
  console.log('  ✅ devotions.html — injected ' + devotions.length + ' devotion(s)');
} else {
  console.log('  ⚠️  devotions.html not found — skipping injection');
}
