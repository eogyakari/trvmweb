    ...
    <div>
      ...
      <section id="books" style={{ padding: '72px 24px', background: '#0D0D1A' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <p style={{ color: '#F5A623', fontSize: 12, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 8 }}>{dict.nav.publications}</p>
          <h2 style={sectionTitleStyle}>{dict.pub.books}</h2>
                  ...
                  <div style={{ padding: 24, flex: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ marginTop: 'auto' }}>
                      {book.is_free ? (
                        ...
                        book.pdf_url ? (
                          <button onClick={() => handlePayment(book, book.pdf_url!, msgs)} style={{
                            ...
                            {t.buy} — {book.currency} {book.price?.toFixed(2)}
                          </button>
                        ) : (
      ...
      <section id="magazines" style={{ padding: '72px 24px', background: '#1A0A2E' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <p style={{ color: '#F5A623', fontSize: 12, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 8 }}>{dict.nav.publications}</p>
          <h2 style={sectionTitleStyle}>{dict.pub.magazines}</h2>
          <div style={{ width: 50, height: 3, background: '#F5A623', marginBottom: 48 }} />
                  ...
                  <div style={{ padding: 24, flex: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ marginTop: 'auto' }}>
                      {mag.is_free ? (
                        ...
                        mag.pdf_url ? (
                          <button onClick={() => handlePayment(mag, mag.pdf_url!, msgs)} style={{
                            width: '100%', background: 'linear-gradient(135deg, #F5A623, #E8860A)',
                            color: '#0D0D1A', padding: '10px 0', border: 'none',
                            borderRadius: 6, fontWeight: 700, fontSize: 13,
                            cursor: 'pointer', fontFamily: 'Georgia, serif',
                            ...
                            {t.buy} — {mag.currency} {mag.price?.toFixed(2)}
                          </button>
                        ) : (
                          <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: 12, textAlign: 'center' }}>{t.comingSoonShort}</p>
                        )
      ...
      <section id="newsletters" style={{ padding: '72px 24px', background: '#0D0D1A' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <p style={{ color: '#F5A623', fontSize: 12, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 8 }}>{dict.nav.publications}</p>
          <h2 style={sectionTitleStyle}>{dict.pub.newsletters}</h2>
          <div style={{ width: 50, height: 3, background: '#F5A623', marginBottom: 48 }} />
