...
export default async function Page() {
  ...
  const key = 'care'
  const title = s[`prog_${key}_title`] || 'Care & Philanthropy'
  const subtitle = s[`prog_${key}_subtitle`] || ''
  const content = s[`prog_${key}_content`] || ''
    ...
    <>
      ...
      <div style={{ background: '#f5f0e8', padding: '56px 20px' }}>
        <article style={{ maxWidth: 800, margin: '0 auto', background: 'white', borderRadius: 12, padding: '48px 40px', boxShadow: '0 2px 16px rgba(0,0,0,0.08)' }}>
          {photo && (
            <Link href="/programs" style={{ color: '#1a3a2a', fontSize: 13, fontWeight: 600, textDecoration: 'none', display: 'block', marginBottom: 24 }}>← All Programs</Link>
          )}
          <div style={{ width: 50, height: 3, background: '#F5A623', marginBottom: 32 }} />
          <div style={{ fontSize: '1.05rem', lineHeight: 2, color: '#222', fontFamily: 'Georgia, serif', whiteSpace: 'pre-wrap' }}>
            {content}
          </div>
        </article>
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 20 }}>
            {others.map(p => (
              <Link key={p.key} href={p.href} style={{
                ...
                <h3 style={{ fontWeight: 700, color: '#F5A623', fontSize: '0.95rem', marginBottom: 6 }}>
                  {s[`prog_${p.key}_title`] || p.key}
                </h3>
                <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13, fontStyle: 'italic' }}>
                  {s[`prog_${p.key}_subtitle`] || ''}
                </p>
      ...
      <section style={{ background: 'linear-gradient(135deg, #7B2FBE 0%, #F5A623 100%)', padding: '64px 24px', textAlign: 'center' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.85)', maxWidth: 480, margin: '0 auto 28px', lineHeight: 1.8 }}>
          Your support helps us continue this vital work for God&apos;s kingdom.
        </p>
        <Link href="/contact" style={{ background: 'white', color: '#7B2FBE', padding: '13px 32px', borderRadius: 30, fontSize: 14, fontWeight: 800, letterSpacing: 1, display: 'inline-block', textDecoration: 'none' }}>
          GET IN TOUCH
        </Link>
      </section>
    </>
